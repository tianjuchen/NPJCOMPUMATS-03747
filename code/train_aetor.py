...
def main(image_size=12):
    ...
    x_train = np.reshape(x_train, (-1, train_size, 128, 128, 1))
    ...
    x_test = np.reshape(x_test, (-1, test_size, 128, 128, 1))
    ...
    for epoch in range(n_epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            model.eval()
            with torch.no_grad():
                x_test_batch = x_test_tensor[:100].to(
                    device
                )  # 使用部分测试数据计算验证损失
            ...
            print(
                f"epoch: {epoch}, Train Loss: {avg_train_loss:.3e}, "
    ...
    plt.savefig(f"{address}/convergence.pdf", dpi=800)
    plt.close()
